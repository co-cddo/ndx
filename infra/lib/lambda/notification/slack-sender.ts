...
import { RetriableError, PermanentError, CriticalError, classifyHttpError } from "./errors"
...
export class SlackSender {
  ...
  async send(params: SlackSendParams): Promise<void> {
    ...
    try {
      ...
      metrics.addDimension("alertType", alertType)
      metrics.addDimension("reason", error instanceof Error ? error.name : "UnknownError")
      ...
      try {
        ...
        if (classifiedError instanceof RetriableError) {
          if (attempt < MAX_RETRIES) {
            ...
            throw new RetriableError(`Slack webhook failed after ${attempt + 1} attempts: ${errorMessage}`, {
        ...
        if (error instanceof TypeError || (error as Error).name === "AbortError") {
          ...
          if (attempt < MAX_RETRIES) {
            ...
            throw new RetriableError(`Network timeout after ${attempt + 1} attempts`, { cause: error as Error })
          }
        }

        // Re-throw classified errors (CriticalError, PermanentError, RetriableError)
        if (error instanceof CriticalError || error instanceof PermanentError || error instanceof RetriableError) {
          throw error
        }
