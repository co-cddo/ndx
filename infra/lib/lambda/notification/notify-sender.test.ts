...
describe("sanitizeValue (AC-1.5, AC-1.17)", () => {
  ...
  it("should HTML-encode script tags to prevent XSS", () => {
    expect(sanitizeValue('<script>alert("xss")</script>')).toBe("&lt;script&gt;alert(&quot;xss&quot;)&lt;/script&gt;")
    expect(sanitizeValue("<img src=x onerror=alert()>")).toBe("&lt;img src=x onerror=alert()&gt;")
  })
...
describe("buildSsoUrl (AC-1.20)", () => {
  ...
  it("should throw for UUID with injection attempt", () => {
    expect(() => buildSsoUrl("https://portal.gov.uk", `${validUuid}?redirect=evil.com`)).toThrow(PermanentError)
  })
})
