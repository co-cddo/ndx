...
describe("NdxStaticStack", () => {
  ...
  test("CloudFormation template contains expected resource types", () => {
    ...
    const resourceTypes = new Set(Object.values(resources).map((r: any) => r.Type))
  ...
  describe("Infrastructure configuration", () => {
    ...
    test("Lambda IAM policy grants CloudFront permissions", () => {
      ...
      const cloudFrontPolicy = (policies as PolicyResource[]).find((policy: PolicyResource) => {
        ...
        return statements.some((stmt: PolicyStatement) => {
          ...
          return actions.includes("cloudfront:UpdateDistribution") || actions.includes("cloudfront:GetDistribution")
        })
      })
    ...
    test("S3 bucket policy configured for CloudFront access", () => {
      ...
      const cloudFrontStatement = typedStatements.find(
        (stmt: BucketPolicyStatement) => stmt.Principal?.Service === "cloudfront.amazonaws.com",
      )
