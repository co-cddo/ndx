...
describe("Post-Deployment Smoke Tests", () => {
  ...
  describe("GOV.UK Notify connectivity (AC-8.8)", () => {
    ...
    conditionalTest("can send test email via GOV.UK Notify", async () => {
    ...
    conditionalTest("GOV.UK Notify API is reachable", async () => {
      const apiKey = process.env.NOTIFY_SANDBOX_API_KEY || getRequiredEnv("NOTIFY_API_KEY")
      const client = new NotifyClient(apiKey)
      ...
      try {
        ...
        console.log(`[Smoke Test] Found ${templates.data?.templates?.length || 0} email templates`)
      } catch (error) {
