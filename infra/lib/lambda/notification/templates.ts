 ...
 */
export function detectEnrichmentConflict(
 ...
 */
export function getBudgetDiscrepancy(
  eventBudgetLimit: number,
  enrichedData?: EnrichedData
): string | undefined {
 ...
 */
export function generateBudgetActionLink(
  leaseKey: LeaseKey,
  secret: string,
  portalUrl: string
): string {
...
export function getTemplateId(config: TemplateConfig): string {
  ...
  if (!templateId) {
    throw new PermanentError(
      `Missing environment variable: ${config.templateIdEnvVar}`,
...
export function validateRequiredFields(
  ...
  if (missingFields.length > 0) {
    throw new PermanentError(
      `Missing required personalisation fields: ${missingFields.join(', ')}`,
      {
        eventType,
        missingFields,
 ...
 */
function extractLeaseKey(
  detail: { leaseId?: string | { userEmail: string; uuid: string }; userEmail?: string; principalEmail?: string }
): LeaseKey {
...
export function buildLeaseApprovedPersonalisation(
  ...
  if (portalConfig) {
    ...
    personalisation.portalLink = generatePortalLink(
      leaseKey,
      'view',
      portalConfig.secret,
      portalConfig.url
    ...
    personalisation.budgetActionLink = generateBudgetActionLink(
      leaseKey,
      portalConfig.secret,
      portalConfig.url
    );
...
export function buildPersonalisation(
  ...
  switch (event.eventType) {
    ...
    case 'LeaseRequested':
      personalisation = buildLeaseRequestedPersonalisation(
        event as ValidatedEvent<LeaseRequestedDetail>
      );
      break;
    case 'LeaseApproved':
      personalisation = buildLeaseApprovedPersonalisation(
        event as ValidatedEvent<LeaseApprovedDetail>
      );
      break;
    case 'LeaseDenied':
      personalisation = buildLeaseDeniedPersonalisation(
        event as ValidatedEvent<LeaseDeniedDetail>
      );
      break;
    case 'LeaseTerminated':
      personalisation = buildLeaseTerminatedPersonalisation(
    ...
    case 'LeaseBudgetExceeded':
      ...
      break;
    case 'LeaseExpired':
      personalisation = buildLeaseExpiredPersonalisation(
        event as ValidatedEvent<LeaseExpiredDetail>,
        enrichedData
      );
      break;
    case 'LeaseFrozen':
      personalisation = buildLeaseFrozenPersonalisation(
        event as ValidatedEvent<LeaseFrozenDetail>,
        enrichedData
      );
      break;
    ...
    default:
      throw new PermanentError(
        `Unsupported event type for templates: ${event.eventType}`,
