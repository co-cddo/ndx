...
describe("NdxNotificationStack", () => {
  ...
  describe("Resource Tagging", () => {
    test("Lambda function has project tag", () => {
      template.hasResourceProperties("AWS::Lambda::Function", {
        Tags: Match.arrayWith([Match.objectLike({ Key: "project", Value: "ndx" })]),
      })
    })

    test("Lambda function has component tag", () => {
      template.hasResourceProperties("AWS::Lambda::Function", {
        Tags: Match.arrayWith([Match.objectLike({ Key: "component", Value: "notifications" })]),
      })
    })
        ...
        Targets: Match.arrayWith([
          Match.objectLike({
            Arn: Match.objectLike({
              "Fn::GetAtt": Match.arrayWith([Match.stringLikeRegexp("NotificationHandler")]),
            }),
          }),
        ]),
      })
