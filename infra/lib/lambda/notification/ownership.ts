...
const OPS_EVENTS: NotificationEventType[] = ["AccountCleanupFailed", "AccountQuarantined", "AccountDriftDetected"]
 ...
 */
export function validateEmailFormat(email: string, eventId: string): { valid: boolean; reason?: string } {
 ...
 */
export function extractLeaseKey(event: ValidatedEvent): { userEmail: string; uuid: string } | null {
 ...
 */
export function generateAuditSignature(eventId: string, verificationResult: boolean, timestamp: string): string {
  const data = `${eventId}:${verificationResult}:${timestamp}`
  const secret = process.env.AUDIT_SECRET || "ndx-audit-secret"
  return createHash("sha256").update(`${secret}:${data}`).digest("hex").substring(0, 16)
}
    ...
    if (accountId) {
      ...
      if (accountRecord && accountRecord.ownerEmail) {
        ...
        if (accountOwnerNormalized !== claimedEmailNormalized) {
          ...
          throw new SecurityError("Email does not match account owner", {
            eventId,
          })
        }
      }
