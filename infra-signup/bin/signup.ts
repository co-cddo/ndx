...
const identityStoreId = app.node.tryGetContext("identityStoreId") as string | undefined
  ?? process.env.IDENTITY_STORE_ID
const groupId = app.node.tryGetContext("groupId") as string | undefined
  ?? process.env.GROUP_ID
const ssoInstanceArn = app.node.tryGetContext("ssoInstanceArn") as string | undefined
  ?? process.env.SSO_INSTANCE_ARN
const distributionId = app.node.tryGetContext("distributionId") as string | undefined
  ?? process.env.DISTRIBUTION_ID

// Slack configuration for Chatbot alerts (Story 3.1)
const slackWorkspaceId = app.node.tryGetContext("slackWorkspaceId") as string | undefined
  ?? process.env.SLACK_WORKSPACE_ID
const slackChannelId = app.node.tryGetContext("slackChannelId") as string | undefined
