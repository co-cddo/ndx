...
beforeAll(async () => {
  ...
  try {
    ...
    if (errorMessage.includes("Secrets Manager") || errorMessage.includes("credentials")) {
      console.error("[E2E Setup] Failed to retrieve sandbox credentials")
      console.error("[E2E Setup] Please ensure:")
      console.error("  1. AWS credentials are configured (aws sso login or AWS_* env vars)")
      console.error("  2. Secrets Manager has E2E credentials at /ndx/notifications/e2e-credentials")
