...
describeE2E('GOV.UK Notify E2E Template Tests', () => {
  ...
  describe('LeaseApproved template (AC-8.2, AC-8.3, AC-8.7)', () => {
    ...
    conditionalTest(
      'sends email with all personalisation fields populated',
      async () => {
    ...
    test('placeholder detection utility works correctly', () => {
      ...
      expect(findUnfilledPlaceholders(bodyWithPlaceholders)).toEqual([
        '((userName))',
        '((accountId))',
      ]);
      ...
      expect(() => assertNoPlaceholders(bodyWithPlaceholders)).toThrow(
    ...
    test('body content assertion utility works correctly', () => {
      ...
      expect(() => assertBodyContains(body, ['Missing Value'])).toThrow(
  ...
  describe.skip('Other template tests (requires template IDs)', () => {
    test.each([
      ...
      '%s template sends without unfilled placeholders',
      (eventType, envVar) => {
