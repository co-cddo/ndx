...
describe("Template Validation", () => {
  ...
  describe("validateTemplateRendering (AC-9.10, AC-9.17)", () => {
    ...
    test("returns empty array when all placeholders filled", async () => {
      ...
      const result = await validateTemplateRendering(mockClient, "template-123", "TestEvent", {
        userName: "test-user",
        accountId: "abc123",
      })
    ...
    test("handles API errors gracefully", async () => {
      ...
      const result = await validateTemplateRendering(mockClient, "template-123", "TestEvent", { userName: "test-user" })
  ...
  describe("validateAllTemplatesOnce (AC-9.7, AC-9.8, AC-9.9)", () => {
    ...
    test("skip validation when SKIP_TEMPLATE_VALIDATION is set", async () => {
      ...
      expect(mockLoggerWarn).toHaveBeenCalledWith(expect.stringContaining("SKIPPED"), expect.any(Object))
    })
    ...
    test("emits TemplateValidationFailed metric on mismatch (AC-9.6)", async () => {
      ...
      expect(mockAddMetric).toHaveBeenCalledWith("TemplateValidationFailed", expect.any(String), expect.any(Number))
    })
    ...
    test("logs template version (AC-9.14)", async () => {
      ...
      expect(mockLoggerInfo).toHaveBeenCalledWith("Template validated", expect.objectContaining({ version: 3 }))
    })
  })
