 ...
 */
export function isAllowedDomain(url, domains) {
  try {
 ...
 *
 * @param {string} url - Image URL
 * @returns {string} - Hash filename with extension
 */
 ...
 * @returns {Array<{fullMatch: string, beforeSrc: string, url: string, afterSrc: string}>}
 */
export function extractImageUrls(content) {
  const imgRegex = /<img([^>]*)\ssrc=["']([^"']+)["']([^>]*)>/gi
  const matches = [...content.matchAll(imgRegex)]
  return matches.map(([fullMatch, beforeSrc, url, afterSrc]) => ({
    fullMatch,
    beforeSrc,
    url,
    afterSrc,
 ...
 * @param {Function} [fetchFn=fetch] - Fetch function (injectable for testing)
 * @returns {Promise<string>} - Local URL path
 */
export async function fetchAndCacheImage(url, config, cache, fetchFn = fetch) {
...
export default function remoteImagesPlugin(eleventyConfig, options = {}) {
  ...
  eleventyConfig.addTransform("remote-images", async function (content) {
    ...
    const externalImages = matches.filter(([, , url]) => isAllowedDomain(url, config.domains))
