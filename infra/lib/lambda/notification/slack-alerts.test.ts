...
function createAccountCleanupFailedEvent(): ValidatedEvent<{
  ...
  return {
    ...
    detail: {
      ...
      cleanupExecutionContext: {
        executionArn:
          'arn:aws:states:eu-west-2:123456789012:execution:cleanup-sm:exec-123',
...
describe('Slack Alerts Processor', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (SlackSender.getInstance as jest.Mock).mockResolvedValue(
      createMockSlackSender()
    );
  });
  ...
  describe('Error Handling', () => {
    it('should throw PermanentError for unknown alert type', async () => {
      ...
      await expect(processSlackAlert(event)).rejects.toThrow(
        'Unknown Slack alert type'
      );
    });
  ...
  describe('getSlackTemplate', () => {
    ...
    it('should return undefined for invalid alert type', () => {
      const template = getSlackTemplate(
        'InvalidType' as 'AccountQuarantined'
      );
