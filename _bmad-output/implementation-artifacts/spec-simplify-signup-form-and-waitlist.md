...
- 2026-05-28 — Follow-up sprint `feat/signup-blocklist-and-cleanups` landed: closed all four deferred test-hygiene items (squatting WARN discriminator field, timing-test useRealTimers, createUserOnly empty-domain log, strengthened silent-strip extras test with mocked SDK payload diff); fixed the pre-existing CDK synth failure in `signup-stack.test.ts` (root cause: Yarn 4 doesn't auto-resolve binaries via `yarn run` — added an explicit script to both workspace package.jsons); added `.husky/_/` to `.prettierignore`; **added new feature** — server-side blocklist of personal email providers (Gmail, Hotmail, Yahoo, etc.) and disposable providers (Mailinator, 10minutemail, etc.) returning 400 `WORK_EMAIL_REQUIRED` with copy "Use your public sector work email address. Personal and disposable email addresses aren't accepted." Client-side parity for the personal subset via `@ndx/signup-types/blocklist-data`. Suffix-match blocklist (asymmetric to allowlist's exact-match); fail-closed startup guard against degraded `disposable-email-domains` shipments; blocklist runs BEFORE USER_EXISTS check (shrinks the enumeration oracle surface flagged in Ask First line 43); timing-floor normalised on the blocked path. New error code `WORK_EMAIL_REQUIRED`; `DOMAIN_NOT_ALLOWED` marked `@deprecated`. Page intro copy clarified: "NDX:Try is currently open to UK local authorities. Other public sector organisations can join the waitlist." Email hint updated to "Enter your public sector work email address".
