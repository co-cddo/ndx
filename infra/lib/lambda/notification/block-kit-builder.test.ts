...
const ALERT_TYPES = ["AccountQuarantined", "LeaseFrozen", "AccountCleanupFailed", "AccountDriftDetected"] as const
...
describe("escapeSlackMrkdwn", () => {
  ...
  it("should handle HTML-like content", () => {
    expect(escapeSlackMrkdwn('<div class="test">content</div>')).toBe('&lt;div class="test"&gt;content&lt;/div&gt;')
  })
})
...
describe("buildContextBlock", () => {
  ...
  it("should include Slack alert version", () => {
    const context = buildContextBlock("evt-123")
    expect(context.elements[0].text).toContain(`Slack Alert ${SLACK_ALERT_VERSION}`)
  })
...
describe("buildActionsBlock", () => {
  ...
  it("should apply button style", () => {
    const actions: ActionLink[] = [{ label: "Danger", url: "https://example.com", style: "danger" }]
    const block = buildActionsBlock(actions)
    expect(block?.elements[0].style).toBe("danger")
  })

  it("should truncate long button labels", () => {
    const actions: ActionLink[] = [{ label: "a".repeat(100), url: "https://example.com" }]
    const block = buildActionsBlock(actions)
...
describe("buildBlockKitPayload", () => {
  ...
  it("should include section block with account ID", () => {
    const payload = buildBlockKitPayload(baseParams)
    const sectionBlock = payload.attachments[0].blocks.find((b) => b.type === "section")
    expect(sectionBlock).toBeDefined()
  })

  it("should include context block with event ID (AC-2.5)", () => {
    const payload = buildBlockKitPayload(baseParams)
    const contextBlock = payload.attachments[0].blocks.find((b) => b.type === "context")
  ...
  it("should include actions block when action links provided (AC-2.4)", () => {
    ...
    const actionsBlock = payload.attachments[0].blocks.find((b) => b.type === "actions")
  ...
  it("should include divider block", () => {
    const payload = buildBlockKitPayload(baseParams)
    const dividerBlock = payload.attachments[0].blocks.find((b) => b.type === "divider")
...
describe("validateBlockKitPayload", () => {
  ...
  it("should throw on missing text field", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Block Kit payload missing required text field")
  })

  it("should throw on missing attachments", () => {
    const payload = { text: "test" } as unknown as SlackBlockKitPayload
    expect(() => validateBlockKitPayload(payload)).toThrow("Block Kit payload missing required attachments array")
  })
  ...
  it("should throw on empty attachments array", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Block Kit payload has empty attachments array")
  })
  ...
  it("should throw on missing attachment color", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Attachment missing required color field")
  })
  ...
  it("should throw on missing attachment blocks", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Attachment missing required blocks array")
  })
  ...
  it("should throw on block missing type", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Block missing required type field")
  })
  ...
  it("should throw on header block missing text", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Header block missing text")
  })
  ...
  it("should throw on empty section block", () => {
    ...
    expect(() => validateBlockKitPayload(payload)).toThrow("Section block must have text or fields")
  })

  it("should throw on context block without elements", () => {
    const payload = {
      text: "test",
      attachments: [{ color: "#FF0000", blocks: [{ type: "context", elements: [] }] }],
    } as unknown as SlackBlockKitPayload
    expect(() => validateBlockKitPayload(payload)).toThrow("Context block must have elements")
  })

  it("should throw on actions block without elements", () => {
    const payload = {
      text: "test",
      attachments: [{ color: "#FF0000", blocks: [{ type: "actions", elements: [] }] }],
    } as unknown as SlackBlockKitPayload
    expect(() => validateBlockKitPayload(payload)).toThrow("Actions block must have elements")
  })
