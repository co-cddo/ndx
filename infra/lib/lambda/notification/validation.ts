...
export const EmailSchema = z
  ...
  .refine(
    (val) => !val.match(/\.{2,}/),
    'Email contains consecutive dots (invalid format)'
  )
  .refine(
    (val) => val.length <= 254,
    'Email exceeds maximum length (254 characters)'
  );
...
export const AccountIdSchema = z
  .string()
  .regex(/^[0-9]{12}$/, 'Invalid AWS account ID - must be exactly 12 digits');
...
export function validateEvent<T = unknown>(
  event: EventBridgeEvent<unknown>
): ValidatedEvent<T> {
